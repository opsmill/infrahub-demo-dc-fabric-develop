...
async def create_pool(client: InfrahubClient, log: logging.Logger, branch: str):
    ...
    # On one single site
    # Get prefix
    # Create resource pool with prefix
    # Create VLAN resource pool
    default_ip_namespace = await client.get(kind="IpamNamespace", name__value="default")
    management_prefix = await client.get(
        kind="InfraPrefix", prefix__value="172.16.0.0/24"
    )

    management_pool = await client.create(
        kind="CoreIPAddressPool",
        name="Management addresses pool for FRA05",
        default_address_type="InfraIPAddress",
    ...
    vlan_id_pool = await client.create(
        kind="CoreNumberPool",
        name="L2 network service vlan id",
        node="InfraVLAN",
...
async def run(
    ...
    log.info("Generation resource pool ...")
    await create_pool(client=client, branch=branch, log=log)
